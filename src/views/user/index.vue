  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="手机号" width="200">
        <template slot-scope="scope">
          {{ scope.row.pass.phone }}
        </template>
      </el-table-column>
      <el-table-column label="邮箱" width="200">
        <template slot-scope="scope">
          {{ scope.row.pass.email }}
        </template>
      </el-table-column>
      <el-table-column class-name="status-col" label="权限" width="150" align="center">
        <template slot-scope="scope">
          <el-tag :type="scope.row.pass.accessLevel | statusFilter">{{ scope.row.pass.accessLevel | statusFilterText }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="permission" label="操作" width="300" align="center">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="success" @click="recharge(scope.row.id)">充值</el-button>
          <el-button size="mini" type="danger" @click="changePsw(scope.row.id)">改密
          </el-button>
          <el-button
            v-if="scope.row.pass.accessLevel < 2"
            size="mini"
            type="info"
            @click="openBanTimeDialog(scope.row.id,scope.row.banTime)"
          >封禁
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="banTimeDialogVisible" width="450px" title="封禁用户" @close="banTimeDialogVisible = false">
      <el-form :inline="true">
        <el-form-item label="封禁时间">
          <el-date-picker v-model="banTime" type="date" placeholder="选择封禁日期" />
        </el-form-item>
        <el-form-item>
